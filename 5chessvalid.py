def validate_board(board_dict):
    # check for black and white king
    if ('bking' not in board_dict.values() or 'wking' not in board_dict.values()) :
        return False

    # check if more than 1 of either kings.
    black_king = 0
    white_king = 0
    for k in board_dict.values():
        if k == 'bking':
            black_king += 1
        elif k == 'wking':
            white_king += 1
...
print(validate_board({"a1": "wking", "a2": "wking", "c3": "bbishop", "d1": "bking"}))  # False: cannot have 2 white kings
